...
class TwitterSimulationRunner:
    ...
    def _get_active_agents_for_round(
        ...
        rng = random.Random(f"{simulation_id}:twitter:{round_num}")
    ...
    async def run(self, max_rounds: int = None, resume: bool = False):
        ...
        Args:
            ...
            resume: 是否从 round_checkpoint.json 记录的断点续跑（跳过数据
                库删除和初始事件播种，从上次记录的round继续，并恢复
                last_rowid/总动作数）。
        """
        print("=" * 60)
        ...
        if resume:
            resume_state = load_resume_state(self.simulation_dir, "twitter")
            if not resume_state["found"]:
                print(
                    "错误: 请求续跑(--resume)但未在 round_checkpoint.json "
                    "中找到 twitter 平台的记录，拒绝继续（避免跳过初始事件"
        ...
        if not resume:
            ...
            await clear_checkpoint_platform_section(self.simulation_dir, "twitter")
